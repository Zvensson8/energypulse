        ...
        <ScrollArea className="max-h-[calc(90vh-9rem)] px-5 py-4">
          ...
          {data && (
            <div className="space-y-4 pb-4">
              ...
              {/* Var man uppdaterar data */}
              <Section
                icon={<Upload className="h-4 w-4" />}
                title="Uppdatera förbrukningsdata"
                subtitle="Här ändrar du underlaget – sedan räknas resultatet om automatiskt"
              >
                <p className="text-sm leading-relaxed text-muted-foreground">
                  Siffrorna ovan baseras på importerad eller manuellt angiven
                  månadsförbrukning. När du uppdaterar data räknas intensitet,
                  kravgap och klimatrisk om för byggnaden.
                </p>
                <div className="mt-3 grid gap-2 sm:grid-cols-2">
                  <Link
                    href="/import"
                    onClick={() => onOpenChange(false)}
                    className="group flex flex-col gap-2 rounded-xl border border-border bg-secondary/40 p-3 transition hover:border-primary/30 hover:bg-primary/5"
                  >
                    ...
                    <p className="text-xs leading-relaxed text-muted-foreground">
                      Ladda upp CSV eller Excel med månadsdata för en eller
                      flera byggnader. Bäst när du har många månader eller ny
                      period.
                    </p>
                  </Link>
                  <Link
                    href={
                      buildingId
                        ? `/data-edit?building=${buildingId}&year=${year}`
                        : "/data-edit"
                    }
                    onClick={() => onOpenChange(false)}
                    className="group flex flex-col gap-2 rounded-xl border border-border bg-secondary/40 p-3 transition hover:border-primary/30 hover:bg-primary/5"
